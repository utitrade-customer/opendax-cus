namespace :service do
  ...
  task :daemons, [:command] do |_task, args|
    @daemons = %w[rango blockchain cron_job deposit deposit_coin_address withdraw_coin influx_writer barong_sidekiq]
  ...
  desc 'Run the dashboard application'
  task :dashboard, [:command] do |task, args|
    args.with_defaults(:command => 'start')

    def start
      puts '----- Starting the dashboard -----'
      sh 'docker-compose up -d dashboard'
    end

    def stop
      puts '----- Stopping the dashboard -----'
      sh 'docker-compose rm -fs dashboard'
    end
  ...
  task :all, [:command] => 'render:config' do |task, args|
    ...
    def start
      ...
      Rake::Task["service:dashboard"].invoke('start')
    ...
    def stop
      ...
      Rake::Task["service:dashboard"].invoke('stop')
